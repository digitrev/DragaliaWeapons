...
import { AccountAdventurer } from './AccountAdventurer';
import useAccordionStatus from '../../hooks/useAccordionStatus';
interface Props {
  data: AccountAdventurerData[];
}

export const AccountAdventurerList: FC<Props> = ({ data }) => {
  const allExpanded = data.reduce((map, adventurer) => {
    map[adventurer.adventurerId] = true;
    return map;
